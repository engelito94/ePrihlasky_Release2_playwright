...
class BasePage:
    ...
    def _safe_set_files(self, locator: Locator, cesta_k_suboru: str, nazov_prvku: str, timeout: int = 20000, screenshot_on_error: bool = True):
        try:
    ...
    def _expect_visible(self, locator: Locator, message: str, timeout: int = 20000):
        expect(locator, message).to_be_visible(timeout=timeout)

    def _expect_contains_text(self, locator: Locator, text: str, message: str, timeout: int = 20000):
        expect(locator, message).to_contain_text(text, timeout=timeout)

    def _expect_exact_text(self, locator: Locator, text: str, message: str, timeout: int = 20000):
        expect(locator, message).to_have_text(text, timeout=timeout)

    def _expect_value(self, locator: Locator, value: str, message: str, timeout: int = 20000):
